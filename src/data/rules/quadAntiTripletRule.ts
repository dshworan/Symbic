...
export class QuadAntiTripletRule extends MoveValidator {
  constructor() {
    super(
      'quadantitriplet',
  ...
  private _transposeGrid(grid: number[][], size: number): number[][] {
  ...
  private _checkRows(puzzle: number[][], size: number): HintStep | null {
    for (let row = 0; row < size; row++) {
